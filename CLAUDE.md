...
- **Image attachments**: `--image <path>` (repeatable) and the REPL's `/image <path> [message]` read a local `.png`/`.jpg`/`.jpeg`/`.gif`/`.webp`, base64-encode it (`crates/aura-cli/src/api/images.rs`), and send it as an `image_url` content part after the text; both backends carry it (HTTP serializes `Message.content` as the OpenAI parts array, standalone maps it in `convert_cli_content`). Staged images live in `ui::state::PENDING_IMAGES` until the next message (so `/image` works live mid-stream, queueing any trailing message; Ctrl-C puts a cancelled turn's images back); Tab on the path argument is shell-style completion from `repl/image_complete.rs` (common-prefix extension first, then each match, then the typed text again); rustyline's circular completion drives it at the prompt via `Completer for AuraHelper`, the mid-stream key loop via `image_tab`, and the hint under the frame lists directories and image files; the conversation's `chat_history` keeps the base64 so `--resume` replays the picture
